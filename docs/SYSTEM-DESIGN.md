...
**Closed since the previous version of this table**: the full 32-entry live evaluation harness run (previously listed here as a 7-entry-sample gap) completed overnight against the Ollama fallback path — 29/32 passed, every failure investigated and reported honestly in `docs/EVALUATION.md`. Removed from the table rather than left showing a stale "partial" status.
