...
The full pipeline (extract → clean → chunk → embed → index) was run against the entire real knowledge corpus via the actual `POST /api/ingestion/knowledge-corpus` endpoint, not a synthetic test fixture: all 24 knowledge-corpus documents (policy-forms, endorsements, reference) completed with zero failures, producing 198 chunks, confirmed by cross-checking three independent sources — the endpoint's own response, `GET /api/documents` (MSSQL), and Qdrant's `points_count` (all agree: 198). A spot-checked point's payload matches this schema exactly (`sectionTitle: "7.2 Changes"`, `formVersion: "PAP-2024-STD"`, `effectiveDate: "2024-01-01"`, `pageNumber: 5`). Re-running ingestion against unchanged files was confirmed idempotent (identical `chunkCount`, no reprocessing, unchanged timestamps) via the actual repeated API call, not inferred from the code alone.
